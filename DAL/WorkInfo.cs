...
namespace DAL
{
    class WorkInfo
    {
        ...
        public Model.WorkInfo GetModel(int id)
        {
            ...
            {
                string sql = "select workInfo.*,activityName from workinfo join activity on workinfo.activitId=activity.activityId where workId=@id";
        ...
        public int GetMyCount(string userName)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from workinfo where userNmae=@userName";
